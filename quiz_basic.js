...
if (localStorage.getItem('userName').substring(0,5) == "Guest") {
    ...
    You may take the quiz with this name, but you will not be able to submit your score. Please click the [settings]
    button in the upper right corner to change your display name.`
}
firstTime = true;
if (localStorage.getItem(`scoreB1`)) {
    lastScore = localStorage.getItem(`scoreB1`);
...
main.innerHTML += `, click "Start quiz."<br><br><button onclick="startQuiz()">Start quiz</button><br><br>`
...
function endOfQuiz() {
    ...
    main.innerHTML += `<br><br><button onclick="location.href='aloa_basicmk.html'">Return to instructions</button>`;    

}

function submitted() {localStorage.setItem(`sentB1`,"yes")}